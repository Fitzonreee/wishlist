<?php
@include('partials/nav_logged_in.php');
?>
		<div class="container">
	      <div class="row">
	      <h4 class="form_label">User's Wishlist</h4>
	        <div class="col s12 m3">
	          <div class="card grey lighten-1">
	            <div class="card-content">
	              <img src="assets/imgs/scooter.jpg">
	            </div>
	            <div class="card-action padding_bottom">
	              <a href="#"><span class="black-text title">Scooter</span></a>
	              <a href="#"><i class="material-icons amber-text accent-2-text right">add</i></a>
	        		<!-- CHECKOUT BUTTON -->
					<form>
						<div>
					 	  <button class="waves-effect waves-light btn amber accent-2 black-text center buy" type="submit" name="action"><i class="material-icons left black-text">payment</i>Buy</button>
						</div>
					</form>
	            </div>
	          </div>
	        </div>

	        <div class="col s12 m3">
	          <div class="card grey lighten-1">
	            <div class="card-content">
	              <img src="assets/imgs/scooter.jpg">
	            </div>
	            <div class="card-action padding_bottom">
	              <a href="#"><span class="black-text title">Scooter</span></a>
	              <a href="#"><i class="material-icons amber-text accent-2-text right">add</i></a>
	        		<!-- CHECKOUT BUTTON -->
					<form>
						<div>
					 	  <button class="waves-effect waves-light btn amber accent-2 black-text center buy" type="submit" name="action"><i class="material-icons left black-text">payment</i>Buy</button>
						</div>
					</form>
	            </div>
	          </div>
	        </div>

	        <div class="col s12 m3">
	          <div class="card grey lighten-1">
	            <div class="card-content">
	              <img src="assets/imgs/scooter.jpg">
	            </div>
	            <div class="card-action padding_bottom">
	              <a href="#"><span class="black-text title">Scooter</span></a>
	              <a href="#"><i class="material-icons amber-text accent-2-text right">add</i></a>
	        		<!-- CHECKOUT BUTTON -->
					<form>
						<div>
					 	  <button class="waves-effect waves-light btn amber accent-2 black-text center buy" type="submit" name="action"><i class="material-icons left black-text">payment</i>Buy</button>
						</div>
					</form>
	            </div>
	          </div>
	        </div>

	        <div class="col s12 m3">
	          <div class="card grey lighten-1">
	            <div class="card-content">
	              <img src="assets/imgs/scooter.jpg">
	            </div>
	            <div class="card-action padding_bottom">
	              <a href="#"><span class="black-text title">Scooter</span></a>
	              <a href="#"><i class="material-icons amber-text accent-2-text right">add</i></a>
	        		<!-- CHECKOUT BUTTON -->
					<form>
						<div>
					 	  <button class="waves-effect waves-light btn amber accent-2 black-text center buy" type="submit" name="action"><i class="material-icons left black-text">payment</i>Buy</button>
						</div>
					</form>
	            </div>
	          </div>
	        </div>
	      
	      </div> <!-- end of row -->
		</div> <!-- end of container -->
	</body>
</html>