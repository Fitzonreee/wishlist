...
class User extends CI_Model {
	...
	public function get_user($id){
		$query = "SELECT user_id, first_name, last_name, email, description, user_level, date_created FROM users WHERE user_id = ?";
	...
	private function verify_user($email, $pass){
		...
		if ($this->check_encryption($pass, $temp["salt"]) == $temp['password']) {
			$this->session->set_userdata("user_id", $temp['user_id']);
			return true;
		}
		else return false;
	}
